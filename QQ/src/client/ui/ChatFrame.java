...
public class ChatFrame extends JFrame{
	...
	public void init(){
		...
		shakeBtn.setToolTipText("向好友发送窗口振动");
		...
		sendFileBtn.setToolTipText("向好友发送文件");
		btnPanel.add(sendFileBtn);

		//私聊按钮
		rybqBtn = new JCheckBox("私聊哦！");
		...
		submitBtn.setToolTipText("按Enter键快捷发送消息！");
		...
		onlineCountLbl = new JLabel("在线好友：");
		...
		currentUserPane.add(new JLabel("本人"), BorderLayout.NORTH);
			...
			public void actionPerformed(ActionEvent e) {
				if(rybqBtn.isSelected()){
					...
					if(null == selectedUser){
						otherInfoLbl.setText("想私聊(请选中在线好友列表中某个好友进行私聊啊)...");
					}else if(DataBuffer.currentUser.getId() == selectedUser.getId()){
						otherInfoLbl.setText("当前状态：不能自己跟自己聊天哦");
					}else{
						otherInfoLbl.setText("当前状态：正在与 " + selectedUser.getNickname()
			...
			public void mouseClicked(MouseEvent e) {
				...
				if(rybqBtn.isSelected()){
					if(DataBuffer.currentUser.getId() == selectedUser.getId()){
						otherInfoLbl.setText("警告！：不能自己跟自己聊天哦");
					}else{
	...
	public void loadData(){
		...
		//设置在线用户列表
		onlineCountLbl.setText("在线好友列表("+ DataBuffer.onlineUserListModel.getSize() +")");
		//启动监听服务器消息的线程
	...
	public void sendShakeMsg(){
		...
		if(null != selectedUser){
			if(DataBuffer.currentUser.getId() == selectedUser.getId()){
				JOptionPane.showMessageDialog(ChatFrame.this, "怎么能给自己发送振动呢!",
			...
			JOptionPane.showMessageDialog(ChatFrame.this, "不能群发送振动!只能私发哦",
					"不能发送", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	public void sendTxtMsg(){
		...
		if ("".equals(content)) { //无内容
			...
			if(rybqBtn.isSelected()){  //私聊
				if(null == selectedUser){
					JOptionPane.showMessageDialog(ChatFrame.this, "没有选择私聊对象!选个聊天好友",
							"不能发送", JOptionPane.ERROR_MESSAGE);
					return;
				}else{
					msg.setToUser(selectedUser);
			...
			if(!this.rybqBtn.isSelected()){//群聊
				if(null == selectedUser){
					...
					sb.append("向").append(selectedUser.getNickname())
			...
			//JTextArea中按“Enter”时，清空内容并回到文本消息的首行
	...
	private void sendFile() {
		...
		if(null != selectedUser){
			...
			JOptionPane.showMessageDialog(ChatFrame.this, "不支持给所有在线好友发送文件!请选择单个好友",
					"不能发送", JOptionPane.ERROR_MESSAGE);
		}
	}

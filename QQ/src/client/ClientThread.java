...
public class ClientThread extends Thread {
			...
			while (DataBuffer.clientSeocket.isConnected()) {
				...
				if (type == ResponseType.LOGIN) {
					...
					ChatFrame.onlineCountLbl.setText(
							"在线好友列表("+ DataBuffer.onlineUserListModel.getSize() +")");
					ClientUtil.appendTxt2MsgListArea("【系统消息】好友"+newUser.getNickname() + "上线了！\n");
					...
					ChatFrame.onlineCountLbl.setText(
							"在线好友列表("+ DataBuffer.onlineUserListModel.getSize() +")");
					ClientUtil.appendTxt2MsgListArea("【系统消息】好友"+newUser.getNickname() + "下线了！\n");
	...
	private void toSendFile(Response response) {
		...
		int select = JOptionPane.showConfirmDialog(this.currentFrame,
				"您的好友："+fromName + " ，向您发送了文件 [" + fileName+ "]!\n同意接收吗?",
		...
		try {
			...
			if (select == JOptionPane.YES_OPTION) {
				...
				if (result == JFileChooser.APPROVE_OPTION){
					...
					ClientUtil.appendTxt2MsgListArea("【文件消息】您已同意接收来自好友： "
							+ fromName +" 的文件，正在接收文件 ...\n");
				} else {
					request.setAction("refuseReceiveFile");
					ClientUtil.appendTxt2MsgListArea("【文件消息】您已拒绝接收来自好友： "
							+ fromName +" 的文件!\n");
				}
			} else {
				request.setAction("refuseReceiveFile");
